...
function vmClone(options, callback) {
  var cmd = 'clonevm ';

  options = options || {};

  cmd += options.originalName + ' --name ' + options.name + ' --register';

  logging.info('Clone new VM as ' + options.name + ' from ' + options.originalName);
  vboxmanage(cmd, function (error, stdout) {
    if ( stdout ) {
      logging.info(stdout);
    }
...
function importOva(path, callback) {
  var cmd = 'import ' + path;

  logging.info('Import new vm image from %s', path);

  vboxmanage(cmd, function (err, stdout) {
    if ( callback ) {
      callback(err, stdout);
    }
  });
...
function showInfo(vmname, callback) {
  logging.info('Getting info for VM %s', vmname);

  vboxmanage('showvminfo ' + vmname, function (error, data) {
    var result = {},
        info = data.split('\n')
        ...
        .map(function (line) {
          ...
          return {
            ...
            value: line.slice(separatorIndex + 1).replace(/\s/g,'')
          };
        });
...
function vmDelete (vmname, callback) {
  logging.info('Delete VM %s', vmname);
  vboxmanage('unregistervm --delete ' + vmname, function ( err, stdout ) {
    if ( callback ) {
      callback(err, stdout);
    }
  });
